...
5. 俯瞰走行ビュー: 壁・コーナーの可読性、16方位スナップでコーナリングが滑らかに見えるか(車体サイズ改善待ち、§8)
6. ~~色演算(発光・煙・夕景)の演算あり/なし/市松ディザの見た目比較・妥当性判断~~ → 実施済み、合格(§6)
7. ~~Mode7透視ズームの見た目の妥当性~~ → 実施済み、不合格(§6・§8)、修正後に再確認待ち
...
## 8. 修正対応中の不具合(2026-07-21人間レビュー起因、原因調査済み・修正は未着手)

### 8.1 候補cが表示されない
候補c(検死レポート題材時)はUI層960×540を内部解像度として使う。`computeIntegerScale`はcontainer(ブラウザwindow実寸から header・余白を引いた領域)が960×540未満だと`fits=false`を返し、「収まりません」メッセージへフォールバックする。この閾値は多くの実利用ウィンドウサイズ(特にモバイル、DevTools併用時のデスクトップ)を上回っており、二層構成そのものの実現可能性に関わる仕様上の制約(バグではなく設計の帰結)。詳細と修正案はagmsgでSuuへ報告済み(未実装、人間判断待ち)。

### 8.2 検死レポートの文字が見にくい
`drawPostMortemReport.ts`はPixelMplus10/12(ビットマップ内蔵TTF、10px/12pxのネイティブ解像度でのみシャープに表示される設計)を、`postMortemLayout.ts`が候補ごとのcontentHeightPxに比例して動的計算したフォントサイズ(実測7〜32px、10/12pxと一致するのは偶然のみ)で描画していた。ネイティブサイズと異なる指定は、ブラウザによる拡大縮小・補間を発生させぼやける。加えて、フォントサイズを解像度に比例させる設計自体が「同一文字サイズでどれだけ情報量が入るか」という二層構成比較の本来の目的を損なっていた。修正案はagmsgでSuuへ報告済み(未実装、人間判断待ち)。

### 8.3 俯瞰走行ビューの車体が小さい
車体スプライトの実寸(`carSprite.ts`)は最大でも側面幅16px×高さ6px(content解像度480×270に対し幅の3.3%)。カメラは自機中心に480×270分の視界を映すため、周回コース全体(概算400×140)がほぼ視界に収まる広角框になっており、車体が画面全体に対して小さく見える一因になっている。原因分析と推奨サイズ案はagmsgでSuuへ報告済み(未実装、人間判断待ち)。

### 8.4 Mode 7が「単なるズーム」に見える(傾いて見えない)
`computePerspectiveRowTransforms`の数値自体(行ごとに横スケールaが変化すること、上端ほど横方向のサンプリング範囲が広がること等)はユニットテストで検証済みで、数式上は行ごとに異なるサンプリング幅・奥行きが計算されている。視覚上「単なるズーム」に見える原因として、(a)デモのアニメーションがzoom(全行に共通する一様倍率)だけを揺らしており、行間の相対的な台形比率自体はアニメーション中一定のまま(倍率だけが動くため「ズームしている」という印象が支配的になりやすい)、(b)見取り図ソース(`floorPlanSource.ts`)が部屋を単色矩形で塗った疎な20px間隔グリッドのみで、遠近感を強く伝える「手前から奥へ収束する線」が視覚的に乏しい、の2点を仮説として特定した。実装のバグの可能性も排除できていないため、修正案は独自に確定せずSuuへ報告する。

## 9. 未決事項(人間/Suu判断待ち、docs/phase1-plan.md §13より継承)
