...
作成日: 2026-07-21
担当: brabit(UI・描画・音)
状態: **Phase 1完了(2026-07-22人間承認)**。実スマホ固有の確認(GPU/発熱/タッチ遅延/OS別ブラウザ差)はPhase 6(スマホ最適化)へ繰越(§15参照)
実費: 0 USD(Unit A〜H通算)
...
## 15. Phase 1最終検証(性能3viewport・初回ロード転送量・キーボード操作/縦持ち、2026-07-22)

§11で未確定だった残件のうち、production差分なしで確認可能な範囲をPlaywright(headless Chromium)による読み取り専用の実測・監査で確定した。診断UI追加・production変更はいずれも行っていない。

### 15.1 A) WorstCaseDemo 3viewport性能測定

手法: Playwright+CDP(`Emulation.setCPUThrottlingRate` rate=4)。viewport/DPRは`viewportReport.ts`の既存代表値(`phone-390x844`等)を流用。既存フロー(「開始」→「計測開始」、ウォームアップ2秒+計測10秒)をそのまま実行。

| viewport | DPR | content/倍率 | 品質モード | フレーム数 | p50/p95/p99 | 16.7ms超過(旧固定閾値) | 実質missed-vsync | メモリ |
|---|---|---|---|---|---|---|---|---|
| 縦390×844 | 3 | 270×480/1x | 通常(1px) | 600/600 | 16.70/16.70/16.80ms | 178/600(29.7%) | 0/600(0%) | 11.9→11.9MB、GCなし |
| 横844×390 | 3 | 480×270/1x | 通常(1px) | 600/600 | 16.70/16.70/16.80ms | 176/600(29.3%) | 0/600(0%) | 12.7→12.7MB、GCなし |
| デスクトップ1920×1080 | 1 | 480×270/2x | 通常(1px) | 596/596 | 16.70/16.70/16.80ms | 161/596(27.0%) | 4/596(0.67%) | 11.9→11.9MB、GCなし |

既存DoD閾値との照合: 旧固定16.7ms閾値では3viewportとも27〜30%が「超過」に見えるが、`computeVsyncAwareStats`(Task#17で導入済み)による実質的なmissed-vsyncでは0〜0.67%と僅少であり、実質的に60fps相当を満たす。

**headlessで確認できた範囲**: 3viewportいずれのp50/p95/p99・missed-vsync・メモリ推移・Mode7品質モード維持。
**実機が必要な範囲(未確認)**: headless Chromiumはソフトウェアレンダリング(GPU非加速)の可能性が高く実機のGPU加速Canvasとは異なりうる点、実際のタッチ操作遅延、端末発熱によるスロットリング挙動。これらはPhase 6(スマホ最適化)での再確認候補とし、Phase1の仕様ゲート・共通DoD達成とは分離して扱う。

### 15.2 B) production初回ロード転送量

手法: `npm run build`→`vite preview`(distをそのまま配信、devサーバとは別)→Playwright新規context(キャッシュ実質未使用)でindex.html初回ロードをCDP `Network.loadingFinished`の`encodedDataLength`(圧縮後の実ワイヤーサイズ、ヘッダ込み)で計測。

- HTML: 976 bytes / CSS: 5,636 bytes(gzip) / JS: 190,110 bytes(gzip) / フォント: 0リクエスト(現行index.htmlは参照していないため)
- **合計: 196,722 bytes(192.1KB)、1MB(1,048,576 bytes)未満(判定: OK、余裕約82%)**

`vite preview`の静的配信(sirvベース)はデフォルトでgzipを適用済み。Vite devサーバの転送量ではなく、実際のdist成果物をpreviewで配信した値であり、本番相当の計測として扱う。

### 15.3 C) キーボード到達・focus-visible・色以外の状態表示・縦持ち390×844監査

**静的コード監査**: `outline: none`等のフォーカスリング除去0件、`tabIndex`の負値・上書き0件、onClickハンドラの付与先9箇所すべて`<button>`要素(疑似ボタンなし)。状態表示は既存の記号(▶)+太字・ネイティブcheckbox/radioの視覚状態など色以外の手段が既に併用されている。

**Playwright自動操作による実地確認**:
- Enterキーでタブボタン選択→`aria-selected="true"`に変化: 確認
- Spaceキーでチェックボックス(残響)をトグル: 確認
- ArrowRightキーでRPMスライダー操作(8000→8100): 確認
- select要素へのキーボードフォーカス到達: 確認
- 縦持ち390×844で全6タブ(解像度比較/俯瞰走行/色演算/Mode7/音源/最悪ケース)とも`scrollWidth`が`clientWidth`(390px)を超えない(横overflowなし)
- 全6タブの縦持ちスクリーンショット目視: 文字切れ・要素重なりなし

**実機が必要な範囲(未確認)**: 実際の物理デバイスでのタッチ操作感、フォーカスリングの実視認性(色覚・輝度条件下)、iOS Safari/Android Chrome固有のレンダリング差異。Phase 6での再確認候補とし、Phase1の仕様ゲート・共通DoD達成とは分離して扱う。

### 15.4 D) 試作画面(retro-proto.html等)の保持方針

「V3新UIシェルが代替し、参照中の診断・証拠(Phase1提出物・Task#17回帰診断基盤等)が不要になった論理単位」で削除する方針を妥当と判断した(`docs/phase1-plan.md` §13へ確定事項として記載)。

### 15.5 E) フォントサブセット方針

現行production初回ロードは192.1KB(§15.2、フォント転送0件含む)で1MB未満に十分収まる。将来PixelMplus採用時に実測1MB超過が確認された場合に限り、削減候補を報告してSuu承認を得てから対応する方針とする(`docs/phase1-plan.md` §13へ確定事項として記載)。

**結論**: A〜Eいずれも仕様ゲート・共通DoDの観点では確認完了。ただし実スマホ固有の項目(GPU/発熱/タッチ遅延/OS別ブラウザ差)は未確認のまま残り、Phase 6での再確認候補とする。

## 16. Phase 1完了承認(2026-07-22)

**人間承認: 2026-07-22、「承認」を明示的に受領。Phase 1を完了として正式記録する。**

### 16.1 Phase 1ゲート達成

`docs/phase1-plan.md` §1のゲート「試遊で解像度承認+画面スクショ美術レビュー運用の開始」を達成(§12、解像度承認は`docs/phase1-resolution-comparison.md`、美術レビュー運用は`docs/phase1-art-review.md`、全16項目合格)。

### 16.2 共通DoD

`docs/phase1-plan.md` §12.1の自動確認・§12.2の人間レビュー提出物は、本書§1〜§16の各Unit・各監査(解像度/乱巻き・ガレージ改善/マスターパレット/サンプルベース音源/性能・転送量・キーボード操作)を通じてすべて満たされたことを確認した。直近の`npm run test`(565 tests)・`npm run build`・`npm run lint`・`git diff --check`はすべて成功している。

### 16.3 技術検証チェックリスト(art-spec冒頭)

内部解像度・乱巻き軌跡の可読性・マスターパレットの最終値・色演算許可リストの効果検証・Mode 7層の技術検証・サンプルベース音源の技術検証の**全6項目が[x]**(`docs/art-spec.md`冒頭)。

### 16.4 Phase 6へ繰越す実スマホ固有確認

以下はheadless Chromiumでの確認では代替できない実機固有の項目であり、Phase 1の仕様ゲート・共通DoD達成とは分離したうえで、Phase 6(スマホ最適化)での再確認候補とする(§15.1・§15.3参照)。

- 実機GPU加速Canvasでの描画負荷(headless Chromiumはソフトウェアレンダリングの可能性が高い)
- 実際のタッチ操作遅延・フォーカスリングの実視認性(色覚・輝度条件下)
- 端末発熱によるスロットリング挙動
- iOS Safari/Android Chrome固有のレンダリング差異

### 16.5 引き続き未確定のまま残る事項

`docs/phase1-plan.md` §13より、人間が判断する事項のうち「共通48色の最終値」「1MB超過時のフォントサブセット方針」「Phase1ゲート後の試作画面の削除または保持」「BGM・モーター音の試聴承認」はいずれも本書で確定済み。残るのは実装対象外の将来判断(会場サブパレット16色・塗装バンク等、Phase 2以降のスコープ)のみ。
