...
作成日: 2026-07-23(2026-07-24更新: 人間試遊承認・Phase2完了確定を反映)
担当: brabit_mot3(UI成果・本書統合)、alice_mot3(素材システム・写像・sweep・(d)(e)見積り、`docs/phase2-gate-status.md`要約を統合)
状態: **人間試遊承認済み・Phase2完了**(承認日2026-07-24。本書末尾10節の試遊チェックリスト1〜9、および追加再試遊(ショッピングカート・native dialogモーダル・サルベージ・暗さ・レイヤ)のいずれも人間確認OK。Phase3着手は本完了をもって自動着手するものではなく、別途実装計画を作成しSuu_mot3・Fable・人間の承認を経ること)
...
| `ba36f6d` | feat(ui): 店・在庫のスクロールとカート操作を改善(全行到達可能化・ショッピングカート化・native dialog化) [brabit_mot3] |
| `8e535db` | fix(ui): カートとサルベージ確認の明るさとレイヤを修正(dialog内専用canvas化) [brabit_mot3] |
...
## 6. brabit側成果: 店・在庫・サルベージUI(最終状態、commit `d0aa020`→`ba36f6d`→`8e535db`)

`docs/phase2-ui-shop-plan.md`(v4、Fable技術レビュー条件付き承認・必須修正A〜D反映、Suu_mot3最終レビュー合格、2026-07-23プロジェクトリード人間実装承認)に従い実装した。初回実装(`d0aa020`)後、人間試遊フィードバックを受けて`ba36f6d`(スクロール到達性・ショッピングカート化・native dialog化)、`8e535db`(dialog内専用canvasによる明るさ・レイヤ修正)の2件を追加実装している。本節は最終状態(2026-07-24人間試遊承認時点)を記述する。
...
- DOM操作要素はアクセシビリティ用フォーカス層に限定。Canvas描画(見た目)とDOM操作要素(キーボード操作・スクリーンリーダー用・タッチ操作)を分離する構成とし、通常のTailwind DOM UIとしての新設は行っていない
- カタログ・棚とも一覧は**全行が可視範囲外でもスクロールで到達可能**(初回実装時の「可視範囲外行unmountによる到達不能」不具合を`ba36f6d`で修正)。ロービングtabindex+Arrow循環+スクロールバー表示により、キーボード操作でも全行へ到達できる
- カート内訳(店)・サルベージ確認(在庫)は`useRetroDialog`によるnative `<dialog>`+`showModal()`で真のモーダルとして実装(`ba36f6d`)。ブラウザ標準のinert機構で背面操作を遮断し、Tab/Shift+Tabの循環をダイアログ内に明示的に閉じる。見た目(Canvas描画)はdialogの子として置いた専用canvas(z-index 0)に分離し、操作ボタン層はposition:relative+z-index 10で前面固定する(`8e535db`。native dialogのtop layer構成では通常canvasが`::backdrop`の遮光を受けてしまうため)
...
- **購入方式はショッピングカート方式**(`ba36f6d`。初回実装の「行ごとに都度購入確認」方式から刷新): 各行での操作は「選択」と「カートへ追加」に分離し、複数ファミリー・複数点をカートへ積んでからカート内訳ダイアログを開き、一括購入を確定する。カート確定は`purchaseCart`(§6.4)による**原子的更新**で、全行の検証が通らない限り在庫・残高のどちらも変化しない
...
- カタログ画面と同じ操作体系に統一(`ba36f6d`): 行の「選択」とサルベージ確定操作を分離し、選択後に確認ダイアログ(native dialog)でサルベージを確定する

### 6.4 仮経済閉ループ(セッション内限定、非永続)

- `src/store/shopEconomy.ts`(Zustandに依存しない純関数)+`src/store/shopEconomyStore.ts`(Zustand hook、`persist`未使用)
- 初期所持金1000Gの固定フィクスチャから開始し、カートへの追加→一括購入(原子的更新)→在庫追加、およびサルベージ確認→回収額加算までが実際に動作する。ページreloadで常に初期フィクスチャへ戻る(`localStorage`呼び出し皆無、コード構造上の保証)
- カート一括購入(`purchaseCart`、`ba36f6d`)は全カート行を`validateCartLines`で先に検証してから状態を一度に更新する原子的更新であり、途中の行でのみ失敗する部分適用は発生しない
...
- 残高不足時はカート内訳ダイアログの確定ボタンを無効化し、Canvas・アクセシブルネーム双方に「所持金不足」を表示

## 7. 自動ゲート結果(最終、2026-07-24・commit `8e535db`時点)

- `npm run test`: **844 tests / 61 files 全pass**(初回実装時819件から、`ba36f6d`のカート/ロービングtabindex追加分等を含み増加)
...
- `git diff --check`: 各コミット(`d0aa020`・`ba36f6d`・`8e535db`)前にdiff checkを実施し、いずれも空白エラーなしを確認
- 初回entry転送量(gzip): `index.html` 0.46KB + JS 206.84KB + CSS 5.53KB ≒ **約213KB**(1MB未満)。`dist/fonts`のPixelMplusは店・棚画面初回マウント時のみFontFace APIで遅延ロードする方式のままで、初回entryには含まれない

## 8. Playwright実機検証・人間再試遊の記録

### 8.1 初回Playwright実機検証(2026-07-23、brabit_mot3実施、commit `d0aa020`時点)

横900×600・縦390×844の両方で実施(scratchpad内の隔離npmプロジェクトからPlaywrightを実行。本体`package.json`/lockfileは無変更)。

- 9ファミリー全行へのキーボードフォーカス到達を確認した(閲覧専用3ファミリーの末尾「段ボール」行まで確認)。**ただしこの初回自動検証は、可視範囲外行がunmountされることで到達不能になる不具合(8.2節)を検出できていなかった**。当該不具合は後の人間試遊で発見され、`ba36f6d`で解消された。初回検証時になぜこの不具合が顕在化しなかったかは特定していない
- 残高不足(チタン1200G>初期所持金1000G)で確定ボタンが無効化され、Canvas・aria-label双方に「所持金不足」表示
- 購入フロー(アルミ線50G): ダイアログ表示→確定で正常動作、タイトル・メッセージ・ボタンが重ならず画面内に収まる(**この時点では行ごとの都度購入確認方式。§6.2のとおり、その後`ba36f6d`でショッピングカート方式へ刷新**)
...
### 8.2 人間試遊フィードバックと追加修正(2026-07-23〜2026-07-24)

初回実装後の人間試遊フィードバック(チェックリスト項目3・6・8関連: 可視範囲外行への到達不能)を受け、`ba36f6d`でスクロール到達性・ロービングtabindex・ショッピングカート化・native dialog化を実装した。続く人間再試遊で、native dialogの`::backdrop`によりカート内訳・サルベージ確認の内容が暗く表示され、かつサルベージ確認のボタンがレイヤ順の問題で見えなくなる不具合が指摘され、`8e535db`でdialog内専用canvasへの描画分離(z-index 0)+操作層の明示的前面化(z-index 10)により解消した(§6.1)。

以下の追加再試遊項目がいずれも人間確認OKとなり、2026-07-24付でPhase2完了の最終承認を得た:

- ショッピングカート(選択→カート追加→カート内訳→一括購入の原子的更新)が横画面・縦画面双方で正しく動作すること
- カート内訳・サルベージ確認のnative dialogモーダルが背面操作を遮断し、Tab/Shift+Tabがダイアログ内で循環すること
- サルベージ確認フローに不具合がなく、確認・取消双方が正しく動作すること
- カート内訳・サルベージ確認いずれも内容が暗くならず、通常の店・棚画面と同等の明るさで表示されること
- サルベージ確認の確定・キャンセルボタンがレイヤ順に関わらず常に前面に表示され、クリック・タッチで操作できること
...
- component単位のDOM操作テストは本リポジトリに`@testing-library`等が未導入のため自動化しておらず、Playwright手動実行(8.1節)+人間再試遊(8.2節)で代替した(vitestは純関数側`shopEconomy.ts`・`formatMaterial.ts`・`layout.ts`等のみ自動化)
- `docs/phase2-plan.md` §9・§17の`MaterialTier`型表記不一致は訂正済み(commit `323061e`、4.3節相当。ティアは各素材ファミリー固有のID型の`tierIndex`+`family`で表現)

## 10. 人間試遊チェックリスト(2026-07-24承認、全項目OK)

以下は`npm run dev`起動後、実際にブラウザで操作して確認された項目である。1〜9はいずれも**確認OK**(2026-07-24承認)。

1. **起動**: `npm run dev`でローカルサーバーを起動し、ブラウザで開く。タイトル画面が表示されること — OK
2. **横画面・縦画面**: ウィンドウ幅を横長・縦長それぞれにしてガレージ→カタログ(店)・棚(在庫・サルベージ)を開き、Canvas表示が画面内に収まること(「収まりません」表示が出ないこと) — OK
3. **9ファミリー閲覧**: カタログ画面で導線・被膜・磁石・ギヤ・電池・ブラシ・台紙・ガイドローラー・ボディの9ファミリーすべてが表示され、台紙・ガイドローラー・ボディには「試遊版では閲覧のみ」と表示されること — OK(初回試遊で可視範囲外行への到達不能が指摘され`ba36f6d`で解消、8.2節)
4. **6ファミリー購入**: 導線・被膜・磁石・ギヤ・電池・ブラシのいずれかを選び、カートへ追加→カート内訳ダイアログで一括購入を確定すると所持金が減り在庫が増えること(§6.2のショッピングカート方式) — OK
5. **残高不足**: 所持金を上回る素材(例: チタンギヤ1200G)をカートに入れると、カート内訳ダイアログの確定ボタンが押せず「所持金が不足しています」と表示されること — OK
6. **在庫**: 棚画面で購入した素材・初期フィクスチャの在庫(個体・スタック双方)が表示されること — OK(初回試遊で可視範囲外行への到達不能が指摘され`ba36f6d`で解消、8.2節)
7. **サルベージ取消/確定**: 個体を選択→サルベージ確認ダイアログでキャンセルすると状態が変わらないこと。再度選択→確定すると在庫から消え所持金が増えること(回収額が最低1G以上であること) — OK
8. **キーボード操作**: マウスを使わず、Tabで一覧へ入り、ArrowUp/ArrowDownで全行(閲覧専用・スタック在庫を含む)へ移動(端で循環)、Home/Endで先頭/末尾行へ移動、Tab/Shift+Tabで一覧外の操作(カートへ追加・カートを見る等)へ移動、Enter/Spaceで操作、Escapeで確認ダイアログを閉じられること — OK(ロービングtabindexによる一覧内Arrow移動+一覧外へのTab移動を`ba36f6d`で追加)
9. **reload初期化**: 購入・サルベージを行った後にページを再読み込みし、所持金・在庫が初期状態(所持金1000G、初期フィクスチャ)へ戻ること — OK

### 10.1 追加再試遊(2026-07-23〜2026-07-24、いずれも確認OK)

`ba36f6d`・`8e535db`の追加実装を受けた再試遊。詳細は8.2節。

- ショッピングカート(選択→カート追加→カート内訳→一括購入の原子的更新)の横画面・縦画面双方での動作 — OK
- カート内訳・サルベージ確認のnative dialogモーダル(背面操作の遮断、Tab/Shift+Tabの循環) — OK
- サルベージ確認フロー(確定・取消) — OK
- カート内訳・サルベージ確認の明るさ(通常画面と同等の明るさで表示されること) — OK
- サルベージ確認の確定・キャンセルボタンのレイヤ順(常に前面表示・クリック/タッチ操作可能) — OK

### 10.2 Phase3着手について

本書の完了確定は、Phase3(破壊モード+図鑑、CLAUDE.md実装フェーズ表)への自動着手を意味しない。Phase3着手には別途実装計画を作成し、Suu_mot3による中継・Fableの技術レビュー・プロジェクトリード(人間)の承認を経る必要がある(仕様書§11のマルチエージェント体制、AGENTS.md/CLAUDE.md「実装前に計画を提示し、承認後に着手する」)。
