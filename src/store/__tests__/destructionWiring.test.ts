...
import { advanceDestructionSeScheduler, EMPTY_SE_SCHEDULER_STATE } from '../../retro/destruction/destructionPresentation';
import { findDestructionSeSpec } from '../../retro/audio/destructionSe';
...
beforeEach(() => {
  resetSaveStore();
  // 明示reset(§7): 各テストは必ずrun未開始から始める
  useGameStore.setState({ _runAccumulator: null, _terminalPresentationAccumulator: null });
});
...
// ---------------------------------------------------------------------------
// P3-4 G8: destructionTerminalの最終stepを表示専用に保持する(_terminalPresentationAccumulator)
//
// D04のburningのように**終端stepでしか成立しない状態・event**は、`_runAccumulator`が
// 同stepでnull化されるためUIへ一度も公開されない。表示専用の退避により、HUD・粒子・SEが
// 既存の単一出典(events + destructionState + replaySnapshot)から停止画面中も観測できる。
// ---------------------------------------------------------------------------
describe('G8: terminal presentation accumulator', () => {
  function buyAndEquipG8(materialId: string, family: 'battery' | 'brush' | 'magnet', key: 'batteryItemId' | 'brushItemId' | 'magnetItemId'): void {
    const purchased = useSaveStore.getState().purchaseMaterialAction(materialId as never);
    expect(purchased.ok, `${materialId}の購入に失敗しました`).toBe(true);
    const item = useSaveStore.getState().inventory.items.filter((i) => i.family === family && i.materialId === materialId).at(-1);
    expect(item, `${materialId}がinventoryに見つかりません`).toBeDefined();
  ...
  function setConfigG8(partial: Record<string, unknown>): void {
    const next = { ...useGameStore.getState().config, ...partial };
    expect(useSaveStore.getState().updateProgress({ config: next as never })).toBe(true);
    useGameStore.setState({ config: next as never });
  }

  /** cashを積んだfresh saveへ戻す(course経路のfixtureは素材購入を伴うため)。 */
  function resetSaveStoreWithCashG8(cashG: number): void {
    const fresh = __testOnly.freshBootstrap();
    const seeded = { ...fresh, inventory: { ...fresh.inventory, cashG } };
    // 本ファイルはfake localStorageを設置しているため、gated actionは**永続実体**を読む。
    // in-memoryだけ差し替えると前runの結果(破壊された電池等)が実体側に残る。
    __testOnly.writeV16(seeded);
    useSaveStore.setState({
  ...
  /** hill-climbのcourse経路でterminalまで走らせる(D06/D09の既存fixtureと同一構成)。 */
  function runCourseToTerminalG8(config: Record<string, unknown>, maxStep: number): void {
    buyAndEquipG8('magnet-neodymium', 'magnet', 'magnetItemId');
    buyAndEquipG8('battery-lithium-polymer', 'battery', 'batteryItemId');
    buyAndEquipG8('brush-precious-metal', 'brush', 'brushItemId');
    useGameStore.getState().setGarageSelection({
      chassisId: 'standard', gearId: 'fast', wheelId: 'large', tireId: 'standard',
    } as never);
    useGameStore.getState().selectTrack('hill-climb');
    setConfigG8(config);
    useGameStore.getState().startCourseRun();
    expect(useGameStore.getState()._runAccumulator, 'startCourseRunでrunが開始されていません').not.toBeNull();
    for (let i = 0; i < maxStep; i += 1) {
      useGameStore.getState().stepCourseRun(1 / 120);
      if (useGameStore.getState()._runAccumulator === null) return;
    }
    throw new Error('course runが終端しませんでした');
  }

  /** D04(電池炎上)をproduction入口で終端まで走らせる。 */
  function runD04ToTerminal(): void {
    buyAndEquipG8('battery-lithium-polymer', 'battery', 'batteryItemId');
    setConfigG8({ slitWidthMm: 0 });
    useGameStore.getState().flickStart();
    for (let i = 0; i < 600; i += 1) {
      useGameStore.getState().stepSim(1 / 120);
      if (useGameStore.getState()._runAccumulator === null) return;
    }
    throw new Error('D04が終端しませんでした');
  }

  it('(a) terminal eventと最終stateが退避accumulatorに載る(D04の例、burningはlive側に一度も現れない)', () => {
    runD04ToTerminal();

    const live = useGameStore.getState()._runAccumulator;
    const kept = useGameStore.getState()._terminalPresentationAccumulator;
    expect(live, 'liveは従来どおりnull(二重適用防止)').toBeNull();
    expect(kept, '終端stepのaccumulatorが退避されていません').not.toBeNull();
    // 終端でしか成立しない状態とeventの両方が、単一出典として載っている。
    expect(kept!.destructionState.battery.profile).toBe('lipo');
    if (kept!.destructionState.battery.profile === 'lipo') {
      expect(kept!.destructionState.battery.d04.stage).toBe('burning');
    }
    expect(kept!.events.map((e) => e.mode)).toContain('D04');
    // event cursor/run境界判定の出典であるreplaySnapshotも保持されている。
    expect(kept!.replaySnapshot).toBeDefined();
    expect(kept!.terminalModeCandidates).toContain('D04');

    // --- D03(電池破裂、motor-only) ---
    resetSaveStoreWithCashG8(1000);
    setConfigG8({ slitWidthMm: 0 }); // 初期アルカリの短絡
    useGameStore.getState().flickStart();
    expect(useGameStore.getState()._runAccumulator, 'D03: runが開始されていません').not.toBeNull();
    for (let i = 0; i < 600; i += 1) {
      useGameStore.getState().stepSim(1 / 120);
      if (useGameStore.getState()._runAccumulator === null) break;
    }
    const keptD03 = useGameStore.getState()._terminalPresentationAccumulator;
    expect(keptD03, 'D03の退避が作られていません').not.toBeNull();
    expect(keptD03!.events.map((e) => e.mode)).toContain('D03');
    expect(keptD03!.terminalModeCandidates).toContain('D03');
    expect(keptD03!.replaySnapshot).toBeDefined();
    expect(keptD03!.destructionState.battery.profile).toBe('nonLipo');

    // --- D06(歯欠け全損、course) ---
    resetSaveStoreWithCashG8(5000);
    runCourseToTerminalG8(
      { coilTurns: 150, magnetDistanceMm: 3, brushPressure: 0.2, sandingQuality: 1.0 },
      2400,
    );
    const keptD06 = useGameStore.getState()._terminalPresentationAccumulator;
    expect(keptD06, 'D06の退避が作られていません').not.toBeNull();
    expect(keptD06!.terminalModeCandidates).toContain('D06');
    // 全損eventがevents側にも載っている(isTotalLossがterminal化の条件)。
    expect(keptD06!.events.some((e) => e.mode === 'D06' && e.isTotalLoss === true)).toBe(true);
    expect(keptD06!.destructionState.modes.D06.toothLossCount).toBe(GEAR_TOTAL_TOOTH_COUNT);
    expect(keptD06!.replaySnapshot).toBeDefined();
    ...
    const keptD09 = useGameStore.getState()._terminalPresentationAccumulator;
    expect(keptD09, 'D09の退避が作られていません').not.toBeNull();
    expect(keptD09!.terminalModeCandidates).toContain('D09');
    expect(keptD09!.events.map((e) => e.mode)).toContain('D09');
    expect(keptD09!.destructionState.modes.D09.triggered).toBe(true);
    expect(keptD09!.replaySnapshot).toBeDefined();
  });

  it('(b) 開始成功・reset・setModeで消え、manual abortでは生成されない', () => {
    // D04は電池を破壊するため、遷移ごとにD03(初期アルカリの短絡)で退避を作り直す。
    // D03も同じdestructionTerminal経路であり、退避の生成条件は同一である。
    const arrangeTerminal = () => {
      setConfigG8({ slitWidthMm: 0 });
      useGameStore.getState().flickStart();
      for (let i = 0; i < 600; i += 1) {
        useGameStore.getState().stepSim(1 / 120);
      ...
      throw new Error('終端しませんでした');
    };

    arrangeTerminal();
    expect(useGameStore.getState()._terminalPresentationAccumulator).not.toBeNull();
    useGameStore.getState().resetSim();
    expect(useGameStore.getState()._terminalPresentationAccumulator, 'resetSimで消えていない').toBeNull();

    arrangeTerminal();
    useGameStore.getState().setMode('garage');
    expect(useGameStore.getState()._terminalPresentationAccumulator, 'setModeで消えていない').toBeNull();

    // 新runの**開始成功**で消える。開始が成功したことを先にassertする
    // (失敗時は「変更しない」契約なので、成功を確かめないと空虚な合格になる)。
    // 電池破裂で装備電池が消えるため、次のrunを始める前に買い直す。
    arrangeTerminal();
    buyAndEquipG8('battery-alkaline', 'battery', 'batteryItemId');
    useGameStore.getState().flickStart();
    expect(useGameStore.getState()._runAccumulator, '新runが開始できていない').not.toBeNull();
    expect(useGameStore.getState()._terminalPresentationAccumulator, '新run開始で消えていない').toBeNull();

    // manual abort(進行中runの中断)では生成しない。
    useGameStore.getState().stepSim(1 / 120);
    expect(useGameStore.getState()._runAccumulator).not.toBeNull();
    useGameStore.getState().resetSim();
    expect(useGameStore.getState()._terminalPresentationAccumulator, 'manual abortで生成されている').toBeNull();

    // **開始失敗では変更しない**(S-5「失敗時はgameStoreローカルruntime不変」)。
    // 退避がある状態を作り、電池未装備で開始を失敗させる。
    arrangeTerminal();
    const keptBeforeFailure = useGameStore.getState()._terminalPresentationAccumulator;
    expect(keptBeforeFailure, '前提: 退避がある状態を作れていません').not.toBeNull();
    // 失敗理由をassertして空虚な合格を防ぐ(電池破裂で装備電池が消えている)。
    const failed = useGameStore.getState().beginProductionRun({ kind: 'motorOnly', initialOmega: FLICK_INITIAL_OMEGA }, 1);
    expect(failed.ok, '開始が失敗していません(負例として成立していない)').toBe(false);
    if (!failed.ok) expect(failed.reason).toBe('電池が未装備です');
    // 同一参照のまま残る。
    expect(useGameStore.getState()._terminalPresentationAccumulator, '開始失敗で退避が変化した')
      .toBe(keptBeforeFailure);

    // 入口(flickStart)経由の失敗でも同じであること。
    useGameStore.getState().flickStart();
    expect(useGameStore.getState()._runAccumulator, '失敗したのにrunが開始されている').toBeNull();
    expect(useGameStore.getState()._terminalPresentationAccumulator).toBe(keptBeforeFailure);
  });

  it('(b-2) 通常の物理終端(finished/stalled)では退避しない(過剰設計防止の確定制限)', () => {
    // 初期装備・初期configのtest-runは失速で終わる(destructionTerminalではない)。
    useGameStore.getState().startTestRun();
    let closed = false;
    for (let i = 0; i < 3600; i += 1) {
      useGameStore.getState().stepTestRun(1 / 120);
      if (useGameStore.getState()._runAccumulator === null) { closed = true; break; }
    }
    expect(closed, '走行が終端しませんでした').toBe(true);
    expect(useGameStore.getState().vehicleState.status).not.toBe('running');
    expect(useGameStore.getState()._terminalPresentationAccumulator).toBeNull();
  });

  it('(c) 退避accumulatorを2フレーム連続で渡してもterminal one-shotは1回だけ', () => {
    // D04のSEは継続loop(D02_D04_flame)なので、one-shotの重複検証には
    // 有限尺SE(D03_batteryBurst)を持つD03の終端を使う。
    setConfigG8({ slitWidthMm: 0 }); // 初期アルカリの短絡=D03
    useGameStore.getState().flickStart();
    for (let i = 0; i < 600; i += 1) {
      useGameStore.getState().stepSim(1 / 120);
      if (useGameStore.getState()._runAccumulator === null) break;
    ...
    const kept = useGameStore.getState()._terminalPresentationAccumulator!;
    expect(kept, 'D03の終端で退避が作られていません').not.toBeNull();
    expect(kept.events.map((e) => e.mode)).toContain('D03');
    const input = {
      events: kept.events,
      destructionState: kept.destructionState,
      replaySnapshot: kept.replaySnapshot,
    };

    // 1フレーム目: terminal eventがone-shotとして出る。
    const first = advanceDestructionSeScheduler(EMPTY_SE_SCHEDULER_STATE, input, 0);
    const firstOneShots = first.voices.filter((v) => v.isNew && findDestructionSeSpec(v.id).kind === 'oneShot');
    expect(firstOneShots.length).toBeGreaterThan(0);

    // 2フレーム目: 同じ退避を渡しても新規one-shotは増えない(processedEventCountが効く)。
    const second = advanceDestructionSeScheduler(first.next, input, 0.02);
    const secondOneShots = second.voices.filter((v) => v.isNew && findDestructionSeSpec(v.id).kind === 'oneShot');
    expect(secondOneShots).toEqual([]);
    // 停止画面中も、鳴っている有限尺voiceは正規化対象として返り続ける(鳴らし直しではない)。
    expect(second.voices.some((v) => v.id === 'D03_batteryBurst')).toBe(true);
  });
});
