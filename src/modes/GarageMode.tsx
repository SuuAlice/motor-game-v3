...
export function GarageMode() {
  ...
  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 p-4 pb-12">
    <header className="rounded-3xl bg-slate-900 p-6 text-white shadow-lg">
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-3xl font-black">手作りカー・ガレージ</h2><p className="mt-2 text-sm text-slate-300">部品の実数値を見ながら、コースに合わせて組み替えます。</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setMode('shop')} className="rounded-xl bg-violet-700 px-4 py-2 font-black">カタログ(店)</button><button onClick={() => setMode('inventory')} className="rounded-xl bg-teal-700 px-4 py-2 font-black">棚(在庫・サルベージ)</button><button onClick={() => setMode('encyclopedia')} className="rounded-xl bg-rose-700 px-4 py-2 font-black">失敗図鑑</button><button onClick={() => setMode('testRun')} className="rounded-xl bg-sky-700 px-4 py-2 font-black">テスト走行</button><button onClick={() => setMode('course')} className="rounded-xl bg-emerald-700 px-4 py-2 font-black">コース選択</button></div></div>
    </header>
