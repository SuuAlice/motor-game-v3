...
describe('phase4SessionReducer', () => {
  ...
  it('型紙の複製は値コピーで、初走記録と参照を共有しない', () => {
    const { runner } = stubRunner();
    const copied = reduceAll([...TO_FACTS_FIRST, { kind: 'beginRepair' }], runner);
    expect(copied.stage).toBe('selectRepairSection');
    const first = copied.firstRecord;
    const repaired = copied.repairedRecord;
    expect(first).not.toBeNull();
    expect(repaired).not.toBeNull();
    if (first === null || repaired === null) return;
    // 値は同一(複製で数値が変わらない=自動改善しない)。
    expect(repaired).toEqual(first);
    // 配列そのものが別物であること。
    expect(repaired).not.toBe(first);
    // 各turnも別objectであること。配列だけ複製してturnを共有すると、
    // turn単位の変更が初走記録へ漏れる抜け道が残る。
    expect(repaired).toHaveLength(first.length);
    repaired.forEach((turn, index) => {
      expect(turn).toEqual(first[index]);
      expect(turn).not.toBe(first[index]);
    });
  });
