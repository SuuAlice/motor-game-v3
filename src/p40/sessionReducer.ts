...
import { copyWindingRecord, type WindingRecord } from '../materials/windingRecord';
...
export function phase4SessionReducer(
  ...
  switch (action.kind) {
    ...
    case 'beginRepair': {
      ...
      // 型紙の複製(spec §9.6)。**参照ではなく値でコピーする**——同じ配列・同じturn objectを
      // 持ち回すと、以後の変更が初走記録まで巻き込む抜け道が残る。現行のcommitRepairは
      // 非破壊置換なので今は破損しないが、「複製」の契約は値の分離そのものである。
      return {
        ...state,
        stage: 'selectRepairSection',
        repairedRecord: copyWindingRecord(state.firstRecord),
        rejectReason: null,
      };
    }
    case 'selectSection': {
