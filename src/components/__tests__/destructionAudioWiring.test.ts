...
describe('パーティクルの実画面接続(§8.2のparticle層)', () => {
  ...
  it('live runも終端退避も無いときだけfield/cursorを消す(G8)', () => {
    // 停止画面中は終端stepの退避を表示入力に使う。両方nullのときだけクリアする。
    expect(effectsCode).toContain('state._runAccumulator ?? state._terminalPresentationAccumulator');
    expect(effectsCode).toContain('if (accumulator === null) {');
    expect(effectsCode).toContain('clearField()');
  });
