...
v2 Phase 5まで完了しています。反射慣性による車体連成、5コース、ガレージ、工作工程、車体対応診断、実験ノート、車体込みレシピコード、実験室、音・演出・PWA・スマホ最適化を実装済みです。最終ゲートは206テスト、build、lintがすべて成功しています。今後の変更は完成後の保守または独立した追加フェーズとして扱います。
...
- `src/components/` / `src/modes/` — ガレージ、工作、テスト走行、5コース、診断、実験室のUI一式
- `src/data/` — コース、車体パーツ、故障車プリセット

このリポジトリはv2(「走れ!手作りモーターカー」、仕様書v4)専用です。v1.5を凍結済みの基準点とし、承認済みの `docs/spec.md` をv2における唯一の正とします。Phase 0〜5は完了済みです。移植時の基準は`docs/baseline-v1.5.md`、完了時点の実装判断は`docs/handoff.md`を参照してください。
...
「走れ!手作りモーターカー」は、中学理科の定番工作である手作りDCモーターを車体へ載せ、ギヤ・車輪・質量・グリップとの釣り合いを試行錯誤するチューニングゲームです。整流子のショート、接触抵抗、ブラシ圧、巻き数といった工作上の要因を、走行負荷のあるシミュレーションとして提示します。
